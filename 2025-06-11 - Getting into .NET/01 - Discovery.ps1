...
Get-Date
$date = Get-Date -Date "7/2/2025"
$date.GetType().FullName

Get-Item '.\'
(Get-Item '.\').GetType().FullName

(Get-Process)[0]
(Get-Process)[0].GetType().FullName

Get-Command Get-Command
(Get-Command Get-Command).GetType().FullName

# Using New-Object

New-Object -TypeName datetime -ArgumentList 2025, 06, 11

New-Object -TypeName IO.DirectoryInfo -ArgumentList '.\'
...
[datetime]::DaysInMonth | Get-Member

[datetime]::DaysInMonth(2025, 06)

# using reflection

$method = [datetime]::DaysInMonth
$method.Invoke(2025, 06)
...
[DateTimeOffset]::FromUnixTimeSeconds(1749667786)
